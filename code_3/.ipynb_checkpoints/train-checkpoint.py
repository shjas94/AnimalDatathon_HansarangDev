...
class CowData(Dataset):
    def __init__(self, cfg: SingleModelConfig, transform=None, augmentations=None, mode='train',
                 val_ratio=0.001) -> None:
        super().__init__()
        self.parent_dir = cfg.main_dir  # 경로
        # self.drive_dir = cfg.drive_dir
        self.transform = transform
        self.mode = mode
        image_dir = os.path.join(self.parent_dir, '/DATA/train/images', '*.jpg')
        image_dir2 = os.path.join(self.parent_dir, '/Result/added_data/images', '*.jpg')
        annotation_dir = os.path.join(self.parent_dir, '/DATA/train/annotations', '*.json')
        annotation_dir2 = os.path.join(self.parent_dir, '/Result/added_data/annotations', '*.json')
        images = sorted(glob.glob(image_dir), key=lambda x: int(x.split('/')[4].split('_')[3].split('.')[0]))
        images2 = sorted(glob.glob(image_dir2), key=lambda x: int(x.split('/')[4].split('_')[3].split('.')[0]))
        images += images2
#         print(images)
        annotations = sorted(glob.glob(annotation_dir), key=lambda x: int(x.split('/')[4].split('_')[3].split('.')[0]))
        annotations2 = sorted(glob.glob(annotation_dir2),
                              key=lambda x: int(x.split('/')[4].split('_')[3].split('.')[0]))
        annotations += annotations2
        if mode == 'train':
            self.imgs = images[int(9000 * val_ratio):]
            self.annots = annotations[int(9000 * val_ratio):]
        elif mode == 'val':
            self.imgs = images[:int(9000 * val_ratio)]
            self.annots = annotations[:int(9000 * val_ratio)]
        # self.imgs = imgs
    ...
    def __getitem__(self, index: int):
        ...
        # vis_keypoints(img_rgb,keypoints)
...
def train(cfg, train_tfms=None, valid_tfms=None):
    ...
    for epoch in range(cfg.epochs):
        ...
        with tqdm(valid_dl, total=valid_dl.__len__(), unit="batch") as valid_bar:
            ...
            best_model = model
            save_dir = os.path.join(cfg.save_folder)
...
if __name__ == "__main__":
    ...
    best_model = train(cfg, train_tfms=None, valid_tfms=None)