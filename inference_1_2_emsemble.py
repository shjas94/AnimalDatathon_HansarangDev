...
def accuracy(output, target, thr_PCK, thr_PCKh, dataset, hm_type='gaussian', threshold=0.35):
...
class CowData(Dataset):
    def __init__(self, cfg: SingleModelConfig, transform=None, augmentations=None, mode='train',
                 val_ratio=0.12) -> None:
        super().__init__()
        ...
        if self.mode == "train" or self.mode == "val":  # /DATA/train/images/
            ...
            annotations = sorted(glob.glob(annotation_dir),
                                 key=lambda x: int(x.split('/')[4].split('_')[3].split('.')[0]))
        elif self.mode == "test":  # /DATA/test/images/
            ...
            annotations = sorted(glob.glob(annotation_dir),
                                 key=lambda x: int(x.split('/')[5].split('_')[3].split('.')[0]))

        if mode == 'train':
            self.imgs = sorted(images, key=lambda x: int(x.split('/')[4].split('_')[3].split('.')[0]))[
                        int(len(images) * val_ratio):]
            self.annots = sorted(annotations, key=lambda x: int(x.split('/')[4].split('_')[3].split('.')[0]))[
                          int(len(annotations) * val_ratio):]
        elif mode == 'val':
            self.imgs = sorted(images, key=lambda x: int(x.split('/')[4].split('_')[3].split('.')[0]))[
                        :int(len(images) * val_ratio)]
            self.annots = sorted(annotations, key=lambda x: int(x.split('/')[4].split('_')[3].split('.')[0]))[
    ...
    def __getitem__(self, index: int):
        ...
        if self.mode != "test":
            ...
            keypoints = np.concatenate([keypoints, np.ones((17, 1))],
                                       axis=1)  # reshape and concatenate to generate heatmap
            ################
...
def test(cfg):
    ...
    yaml_path = os.path.join(cfg.code_dir, 'Config', yaml_name)
    model1 = model_define(yaml_path)
    model2 = model_define(yaml_path)
    model1 = model1.to(device)
    model2 = model1.to(device)
    model1.load_state_dict(torch.load('./weights/js_model.pth', map_location=device))
    checkpoint = torch.load('./weights/cw_model.pt', map_location=device)
    ...
    for sample in tqdm(test_dl):
        ...
        preds = torch.mean(torch.stack((preds1.detach().cpu(), preds2.detach().cpu()), dim=1), dim=1)
    ...
    with open('./submission_1_2_emsemble.json', 'w') as f:
