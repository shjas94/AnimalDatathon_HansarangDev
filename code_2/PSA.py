...
class PSA_p(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=1, stride=1):
        ...
        self.conv_up = nn.Conv2d(
            self.inter_planes, self.planes, kernel_size=1, stride=1, padding=0, bias=False)
        self.softmax_right = nn.Softmax(dim=2)
        self.sigmoid = nn.Sigmoid()

        self.conv_q_left = nn.Conv2d(
            self.inplanes, self.inter_planes, kernel_size=1, stride=stride, padding=0, bias=False)  # g
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv_v_left = nn.Conv2d(self.inplanes, self.inter_planes,
                                     kernel_size=1, stride=stride, padding=0, bias=False)  # theta
    ...
    def channel_pool(self, x):
        ...
        # [N, IC, H*W]
        theta_x = self.conv_v_left(x).view(
...
class PSA_s(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=1, stride=1):
        ...
        self.conv_up = nn.Sequential(
            nn.Conv2d(self.inter_planes, self.inter_planes //
                      ratio, kernel_size=1),
