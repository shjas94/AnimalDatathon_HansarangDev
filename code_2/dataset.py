...
class CowData(Dataset):
    def __init__(self, cfg: SingleModelConfig, transform=None, augmentations=None, mode='train', val_ratio=0.2) -> None:
        ...
        self.mode = mode
        image_dir = os.path.join('/DATA/train', 'images', '*.jpg')
        annotation_dir = os.path.join('/DATA/train', 'annotations', '*.json')
        images = sorted(glob.glob(image_dir))
        annotations = sorted(glob.glob(annotation_dir))
        if mode == 'train':
            self.imgs = sorted(glob.glob(os.path.join('/DATA/train', 'images', '*.jpg'))  # path만 수정
                               )[int(9000*val_ratio):]
            self.annots = sorted(glob.glob(os.path.join(
                '/DATA/train', 'annotations', '*.json')))[int(9000*val_ratio):]
        elif mode == 'val':
            self.imgs = sorted(glob.glob(os.path.join(
                '/DATA/train', 'images', '*.jpg')))[:int(9000*val_ratio)]
            self.annots = sorted(glob.glob(os.path.join(
                '/DATA/train', 'annotations', '*.json')))[:int(9000*val_ratio)]
        # self.imgs = imgs
        # self.annots = annots
    ...
    def __getitem__(self, index: int):
        ...
        img = cv2.imread(os.path.join(
            '/DATA/train/', 'images', img_info['file_name']))
